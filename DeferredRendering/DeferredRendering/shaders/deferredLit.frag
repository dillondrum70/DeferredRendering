...
void pointLights(inout vec3 diffuse, inout vec3 specular, vec3 position, vec3 normal)
{
    for(int i = 0; i < _UsedPointLights; i++)
    {
        ...
        float dist = distance(position, _PointLight[i].pos);    //distance between candidate point and light
        vec3 lightDir = normalize(_PointLight[i].pos - position);  //Direction to light
...
{   
    ...
    pointLights(diffuse, specular, position, normal);
